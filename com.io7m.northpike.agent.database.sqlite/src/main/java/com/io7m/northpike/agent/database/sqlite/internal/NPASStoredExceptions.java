...
import com.io7m.northpike.agent.database.sqlite.internal.cb.NPDBException;
import com.io7m.northpike.agent.database.sqlite.internal.cb.NPDBStackTraceElement;
...
public final class NPASStoredExceptions
{
  ...
  {
    return exceptionOpt.map(NPASStoredExceptions::serializeException)
      .orElse(null);
  }
    ...
    final NPStoredException e)
  {
    return new NPDBException(
      ...
      CBOptionType.fromOptional(
        e.cause()
          .map(NPASStoredExceptions::mapExceptionToWire)
      ),
      CBLists.ofCollection(
        e.suppressed(), NPASStoredExceptions::mapExceptionToWire),
      CBLists.ofCollection(
        e.stackTrace(), NPASStoredExceptions::mapStackElementToWire)
    );
  }
    ...
    final NPDBException e)
  {
    return new NPStoredException(
      ...
      e.fieldCause()
        .asOptional()
        .map(NPASStoredExceptions::mapExceptionFromWire),
      CBLists.toList(
        e.fieldSuppressed(),
        NPASStoredExceptions::mapExceptionFromWire
      ),
      CBLists.toList(
        e.fieldStackTrace(),
        NPASStoredExceptions::mapStackElementFromWire
      )
    );
  }
