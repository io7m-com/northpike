...
public final class NPAShellCmdSet extends NPShellCmdAbstract
{
  enum Formatter
  {
    RAW,
    ...
    context.parameterValue(FORMATTER)
      .ifPresent(r -> {
        switch (r) {
          case RAW -> {
            this.options()
              .setFormatter(new NPFormatterRaw(this.terminal()));
          }
          case PRETTY -> {
            this.options()
              .setFormatter(new NPFormatterPretty(this.terminal()));
          }
        }
      });
