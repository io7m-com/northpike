...
/**
 * @see NPUCommandRepositorySearchNext
 */

public final class NPUCmdRepositorySearchNext
  extends NPUCmdAbstract<NPUResponseRepositorySearch, NPUCommandRepositorySearchNext>
{
  /**
   * @see NPUCommandRepositorySearchNext
   */

  public NPUCmdRepositorySearchNext()
  {
    super(NPUCommandRepositorySearchNext.class);
  }

  @Override
  public NPUResponseRepositorySearch execute(
    final NPUserCommandContextType context,
    final NPUCommandRepositorySearchNext command)
    throws NPException
  {
    ...
    try (var transaction = context.transaction(NORTHPIKE_READ_ONLY)) {
      ...
      return new NPUResponseRepositorySearch(
        UUID.randomUUID(),
        command.messageID(),
        paged.pageNext(transaction).map(NPRepositoryDescription::summary)
      );
    }
  }
}
