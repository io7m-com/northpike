...
/**
 * The socket service for users.
 */

public final class NPUserServerSocketService
  implements NPUserServerSocketServiceType
{
  private final ServerSocketFactory sockets;

  private NPUserServerSocketService(
  ...
  public static NPUserServerSocketServiceType create(
    final NPTLSContextServiceType tlsService,
    final NPServerUserConfiguration configuration)
    throws NPServerException
  {
    ...
    if (tls instanceof NPTLSDisabled) {
      return new NPUserServerSocketService(
  ...
  {
    return "User server socket service.";
  }
}
