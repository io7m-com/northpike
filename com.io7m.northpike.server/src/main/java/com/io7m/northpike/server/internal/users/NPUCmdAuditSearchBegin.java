...
/**
 * @see NPUCommandAuditSearchBegin
 */

public final class NPUCmdAuditSearchBegin
  extends NPUCmdAbstract<NPUResponseAuditSearch, NPUCommandAuditSearchBegin>
{
  /**
   * @see NPUCommandAuditSearchBegin
   */

  public NPUCmdAuditSearchBegin()
  {
    super(NPUCommandAuditSearchBegin.class);
  }

  @Override
  public NPUResponseAuditSearch execute(
    final NPUserCommandContextType context,
    final NPUCommandAuditSearchBegin command)
    throws NPException
  {
    ...
    NPSecurity.check(
      user.userId(),
      user.subject(),
      NPSecObject.AUDIT_LOG.object(),
      NPSecAction.READ.action()
    );

    try (var transaction = context.transaction(NORTHPIKE_READ_ONLY)) {
      final var paged =
        transaction.queries(NPDatabaseQueriesAuditType.EventSearchType.class)
          .execute(command.parameters());

      context.setProperty(NPAuditPagedType.class, paged);
