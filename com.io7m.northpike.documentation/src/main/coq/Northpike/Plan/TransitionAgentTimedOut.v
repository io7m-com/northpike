...
<: Transition.S P Maps State.
  ...
  Lemma waitingForAgentDec : forall s e,
    {EWaitingForAgent 0 = statusOf s e}+{~EWaitingForAgent 0 = statusOf s e}.
  Proof.
    move => s e.
    elim (statusOf s e).
  ...
  (** The "waiting for agent timed out" precondition. An element can time out
      waiting for an agent if:
      - The element is a task.
      - The status of the element is _EWaitingForAgent 0_.
  *)
  ...
  : Prop :=
       P.elementIsTask e
    /\ EWaitingForAgent 0 = statusOf s e.

  (** The state transitions to a new state where the element has timed out. *)
  ...
  Proof.
    ...
    elim (P.elementIs e).
    - move => Ht.
      elim (waitingForAgentDec s e) => [HL|HR].
      * left; intuition.
      * right; intuition.
    - move => Hb.
      right.
