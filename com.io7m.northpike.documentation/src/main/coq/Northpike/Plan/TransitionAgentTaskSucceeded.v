...
<: Transition.S P Maps State.
  ...
  (** The "agent succeeded executing" precondition. A task can succeed if
      if:
  ...
  *)
  ...
  (** The state transitions to a new state where the task has succeeded. *)
  ...
  : state :=
    let s1 := withStatus s e ESuccess in
      withAgentStatus s1 (agentOf a) (ASSucceeded e).
