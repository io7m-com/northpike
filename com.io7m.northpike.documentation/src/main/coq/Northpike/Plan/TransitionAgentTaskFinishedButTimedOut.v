...
<: Transition.S P Maps State.
  ...
  Lemma failedDec : forall s e,
    {EFailureTimedOut = statusOf s e}+{~EFailureTimedOut = statusOf s e}.
  Proof.
    move => s e.
    elim (statusOf s e).
  ...
  (** The "task execution timed out" precondition. This state transition can
      occur if:
      - The element _e_ is a task.
      - The status of the element is _EFailureTimedOut_.
      - The status of the agent is _ASExecuting e_.

      This state transition encodes the notion of an agent finishing a task
      but then finding out that the task has already been considered to have
      taken too long and timed out.
  *)
  ...
  : Prop :=
       P.elementIsTask e
    /\ EFailureTimedOut = statusOf s e
    /\ ASExecuting e = agentStatusOf s a.
  ...
  Proof.
    ...
    - elim (P.elementEqDec e0 e1) => [HeL|HeR].
      ...
      * subst.
        ...
        ** contradict Hp2. rewrite <- HL; congruence.
        ** contradict Hp2; rewrite <- HR; congruence.
    - elim (P.elementEqDec e0 e1) => [HeL|HeR].
      * subst.
        rewrite <- statusOfIndependence in Hb.
        rewrite <- Hp1 in Hb.
