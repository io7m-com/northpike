...
<: Transition.S P Maps State.
  ...
  (** The "waiting for dependencies failed" preconditions. An element can fail
      if:
      - The status of the element is _EWaitingForDependencies_.
      - The element has at least one dependency that has failed.
  *)
  ...
  : Prop :=
    ...
    /\ List.Exists (fun f => statusFailed (statusOf s f)) (P.elementDependencies e).

  (** The state transitions to a new state where the element has failed. *)
  Definition transition 
    (s : state)
    (a : arguments)
    (e : P.element)
  : state :=
    withStatus s e EFailure.

  (** For a given element _e_ and state _s_, the "waiting for dependencies failed" 
      preconditions is decidable. *)
  ...
  Proof.
    ...
    have Hle : ({List.Exists (fun f : P.element => statusFailed (statusOf s f)) (P.elementDependencies e)}
             + {~List.Exists (fun f : P.element => statusFailed (statusOf s f)) (P.elementDependencies e)}). {
  ...
  (** For a given element state _s_, it is decidable whether the
      "waiting for dependencies failed" preconditions holds. *)
