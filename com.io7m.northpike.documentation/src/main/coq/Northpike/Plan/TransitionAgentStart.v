...
<: Transition.S P Maps State.
  ...
  (** The "waiting for agent starts" precondition. An element can start
  ...
  *)
  ...
  (** The state transitions to a new state where the element is waiting for an agent. *)
  Definition transition 
    (s : state)
    (a : arguments)
    (e : P.element)
  : state :=
    withStatus s e (EWaitingForAgent P.timeoutAgentWait).
  ...
  Proof.
    ...
    - subst.
      rewrite withStatusOf in Hb.
      assert (n = P.timeoutAgentWait) as Hn by congruence.
      subst.
      auto.
    - rewrite <- withStatusNotOf in Hb.
      apply (Hi4 e1 n Hb).
