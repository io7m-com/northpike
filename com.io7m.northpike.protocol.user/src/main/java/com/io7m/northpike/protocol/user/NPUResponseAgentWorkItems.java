...
/**
 * A work item list retrieval.
 *
 * @param messageID     The message ID
 * @param correlationID The command that prompted this response
 * @param workItems     The currently executing work items
 */

public record NPUResponseAgentWorkItems(
  UUID messageID,
  UUID correlationID,
  ...
  implements NPUResponseType
{
  ...
  public NPUResponseAgentWorkItems
  {
    Objects.requireNonNull(messageID, "messageID");
    Objects.requireNonNull(correlationID, "correlationID");
    Objects.requireNonNull(workItems, "workItems");

    for (final var workItem : workItems) {
      if (workItem.selectedAgent().isEmpty()) {
        throw new NPValidityException(
          "All work items must have a selected agent."
        );
      }
    }
  }
  ...
  public static NPUResponseAgentWorkItems createCorrelated(
