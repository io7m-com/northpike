...
/**
 * @see NPACCommandServerDelete
 */

public final class NPACCmdServerDelete
  extends NPACCmdAbstract<NPACResponseOK, NPACCommandServerDelete>
{
  /**
   * @see NPACCommandServerDelete
   */

  public NPACCmdServerDelete()
  {
    super(NPACCommandServerDelete.class);
  }

  @Override
  public NPACResponseOK execute(
    final NPACCommandContextType context,
    final NPACCommandServerDelete command)
    throws NPException
  {
    ...
    try (var connection = context.databaseConnection()) {
      try (var transaction = connection.openTransaction()) {
        transaction.queries(ServerDeleteType.class).execute(command.server());
        transaction.commit();
        context.publishEvent(new NPAgentServerDeleted(command.server()));
