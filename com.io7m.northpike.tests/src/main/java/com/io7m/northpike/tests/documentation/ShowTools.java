...
public final class ShowTools
{
    ...
    throws Exception
  {
    ...
    for (final var e : ServiceLoader.load(NPToolFactoryType.class)) {
      ...
      t.setTextContent(e.toolName().toString());
      c0.appendChild(t);

      final var c1 = doc.createElementNS(STRUCTURAL_8,"Cell");
      row.appendChild(c1);

      c1.setTextContent(e.description());
    }

    serializeDocument(doc, outputDirectory.resolve("model-tools.xml"));
  }
